...
def bench_ip_parse_v4(mut b: Bencher) raises:
...
def bench_ip_parse_v6(mut b: Bencher) raises:
...
def bench_ip_parse_v4_broadcast(mut b: Bencher) raises:
...
def bench_socket_addr_parse(mut b: Bencher) raises:
...
def bench_socket_addr_parse_ipv6(mut b: Bencher) raises:
...
def bench_ip_localhost(mut b: Bencher) raises:
...
def bench_dns_resolve_localhost(mut b: Bencher) raises:
...
def bench_dns_resolve_v4_localhost(mut b: Bencher) raises:
...
def main() raises:
    ...
    bench.bench_function(
        bench_ip_parse_v4_broadcast, BenchId("IpAddr.parse IPv4 broadcast")
    )
    bench.bench_function(bench_ip_parse_v6, BenchId("IpAddr.parse IPv6"))
    bench.bench_function(
        bench_socket_addr_parse, BenchId("SocketAddr.parse IPv4:port")
    )
    bench.bench_function(
        bench_socket_addr_parse_ipv6, BenchId("SocketAddr.parse [IPv6]:port")
    )
    bench.bench_function(
        bench_ip_localhost, BenchId("IpAddr.localhost() struct init")
    )
    ...
    dns_bench.bench_function(
        bench_dns_resolve_localhost, BenchId("resolve('localhost')")
    )
    dns_bench.bench_function(
        bench_dns_resolve_v4_localhost, BenchId("resolve_v4('localhost')")
    )
